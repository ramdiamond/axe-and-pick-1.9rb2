...
GUIMainWindow::GUIMainWindow() : QObject()
{
    ...
    m_pqtApp = NULL;
    m_pqtMainViewer = NULL;
    m_pqtOpenSavedGameViewer = NULL;
    m_pqtSettingsDialogViewer = NULL;
 ...
 *
 ****************************************/
int GUIMainWindow::Initialize(int argc, char *argv[])
{
...
#endif
    m_pGuiMainWindow = g_pGuiMainWindow;

    m_pqtApp = new QApplication(argc, argv);
    m_pqtMainViewer = new QtQuick2ApplicationViewer();
    m_pqtMainViewer->setMinimumWidth(400);
    m_pqtMainViewer->setMinimumHeight(350);
    //m_pqtMainViewer->setModality(Qt::ApplicationModal);
    ...
    m_pqtMainViewer->setTitle(APP_NAME" v"APP_VERSION);
    m_pqtMainViewer->rootContext()->setContextProperty("myHandler", m_pGuiMainWindow);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("settings", m_pSettings);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("savesAccess", m_pSavesAccess);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("savedGameModel", m_pSavedGameModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("resourceModel", m_pResourceModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("resourceModelProxy", m_pqtProxyResourceModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("humanModel", m_pHumanModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("humanModelProxy", m_pqtProxyHumanModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("neutralMobModel", m_pNeutralMobModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("neutralMobModelProxy", m_pqtProxyNeutralMobModel);

    // Violent Mobs
    m_pViolentMobModel = new ViolentMobListModel(new ViolentMob, qApp);
    m_pSavesAccess->setViolentMobModel(m_pViolentMobModel);
    m_pqtMainViewer->rootContext()->setContextProperty("violentMobModel", m_pViolentMobModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("violentMobModelProxy", m_pqtProxyViolentMobModel);

    // Migrants
    m_pMigrantModel = new HumanListModel(new Human, qApp);
    m_pSavesAccess->setMigrantModel(m_pMigrantModel);
    m_pqtMainViewer->rootContext()->setContextProperty("migrantModel", m_pMigrantModel);
    ...
    m_pqtMainViewer->rootContext()->setContextProperty("migrantModelProxy", m_pqtProxyMigrantModel);

    // Load the QML
    m_pqtMainViewer->setMainQmlFile(QStringLiteral("qml/AxeAndPick/main.qml"));

    // Setup the other dialogs
    m_pqtSettingsDialogViewer = new QtQuick2ApplicationViewer();
    m_pqtSettingsDialogViewer->setMinimumWidth(400);
    m_pqtSettingsDialogViewer->setMinimumHeight(350);
    m_pqtSettingsDialogViewer->setTitle(APP_NAME" v"APP_VERSION" - Settings");
    m_pqtSettingsDialogViewer->setMainQmlFile(QStringLiteral("qml/AxeAndPick/SettingsWindow.qml"));

    // This makes the window a 'child' with 'forced focus' of main view, but also an independant window within the app.
    m_pqtSettingsDialogViewer->setTransientParent(m_pqtMainViewer);
    m_pqtSettingsDialogViewer->setModality(Qt::ApplicationModal);

    m_pqtSettingsDialogViewer->rootContext()->setContextProperty("globalHandler", m_pGuiMainWindow);
    //m_pqtSettingsDialogViewer->rootContext()->setContextProperty("myHandler", m_pGuiSettingsWindow);
...
int GUIMainWindow::Clean()
{
    ...
    if (m_pqtSettingsDialogViewer)  delete m_pqtSettingsDialogViewer;
    m_pqtSettingsDialogViewer = NULL;
    if (m_pGuiSettingsWindow)       delete m_pGuiSettingsWindow;
    m_pGuiSettingsWindow = NULL;

    if (m_pqtOpenSavedGameViewer)   delete m_pqtOpenSavedGameViewer;
    m_pqtOpenSavedGameViewer = NULL;
    ...
    if (m_pqtMainViewer)            delete m_pqtMainViewer;
    m_pqtMainViewer = NULL;
 ...
 *
 *************************************/
int GUIMainWindow::Main()
{
    int ret = 0;

    // Show the GUI
    m_pqtMainViewer->showExpanded();
...
void GUIMainWindow::OpenSettingsButton()
{
    ...
    m_pqtSettingsDialogViewer->showExpanded();
}

GUIMainWindow *g_pGuiMainWindow = NULL;
